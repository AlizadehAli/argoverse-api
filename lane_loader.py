...
@lru_cache(128)
def _data_read_csv(path: Path, *args: Any, **kwargs: Any) -> pd.DataFrame:
...
@lru_cache(128)
def _lane_read_csv(path: Path, *args: Any, **kwargs: Any) -> pd.DataFrame:
    """A caching CSV reader
...
class DataLaneLoader:
    def __init__(self, root_dir: Union[str, Path], lane_root_dir: Union[str, Path]):
        ...
        lane_root_dir = Path(lane_root_dir)
        ...
        self.lane_seq_list = [(lane_root_dir / x).absolute() for x in os.listdir(str(lane_root_dir))]

        # self.current_seq: Path = self.seq_list[self.counter]
        self._update_current_seq()

    def _update_current_seq(self):
        self.current_seq = self.seq_list[self.counter]
        self.current_lane_seq = self.lane_seq_list[self.counter]
    ...
    def seq_df(self) -> pd.DataFrame:
        ...
        return _data_read_csv(self.current_seq)

    @property
    def lane_df(self) -> pd.DataFrame:
        """Get the dataframe for the current lane sequence.

        Returns:
            pandas DataFrame for the current lane sequence
        """

        return _lane_read_csv(self.current_lane_seq)
    @property
    ...
    def get(self, seq_id: Union[Path, str], lane_seq_id: Union[Path, str]) -> "ArgoverseForecastingLoader":
        ...
        self.current_lane_seq = Path(lane_seq_id).absolute()
        return self
